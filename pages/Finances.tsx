...
import { ExportIcon, ArrowUpRightIcon, ArrowDownLeftIcon } from '../components/Icons';
...
const BarChart: React.FC<{ data: BarChartData[] }> = ({ data }) => {
    ...
    return (
        <Card>
            ...
            <p className="text-sm text-brand-gray-500 mb-6">Últimos 6 meses</p>
             <div className="flex h-56">
                <div className="flex flex-col justify-between text-xs text-brand-gray-500 pr-2 text-right h-48 self-end mb-6">
                    {steps.slice().reverse().map(step => <div key={step}>{step}</div>)}
                </div>
...
const Finances: React.FC<FinancesProps> = ({ data }) => {
    const { stats, incomeVsExpense, incomeByCategory, expenseByCategory, recentTransactions } = data;
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <PieChart title="Receitas por Categoria" subtitle="Distribuição das receitas" data={incomeByCategory} />
                <PieChart title="Despesas por Categoria" subtitle="Distribuição das despesas" data={expenseByCategory} />
